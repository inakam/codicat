...
// 自動生成ファイルを作成するヘルパー関数
fn create_generated_file(repo_dir: &Path) -> Result<()> {
    let file_path = repo_dir.join("generated.rs");
    let mut file = File::create(&file_path)?;

    // 自動生成ファイルのコンテンツ（検出可能なヘッダー付き）
    let content = "// Auto-generated by test runner\n// DO NOT EDIT\n\nfn generated_function() {\n    println!(\"This is generated code\");\n}\n";
    file.write_all(content.as_bytes())?;

    // Gitに追加
    Command::new("git")
        .args(["add", "generated.rs"])
        .current_dir(repo_dir)
        .output()
        .context("Failed to git add generated file")?;

    // コミット
    Command::new("git")
        .args(["commit", "-m", "Add generated file"])
        .current_dir(repo_dir)
        .output()
        .context("Failed to commit generated file")?;
...
fn test_help_output() -> Result<()> {
    ...
    assert!(stdout.contains("--exclude-generated"));
...
#[test]
fn test_exclude_generated_option() -> Result<()> {
    let repo = setup_git_repo()?;
    create_test_files(repo.path())?;
    create_generated_file(repo.path())?;

    // 通常のコマンド実行（自動生成ファイルを含む）
    let (stdout_with_generated, _) = run_codicat_with_args(&["./"], Some(repo.path()))?;

    // --exclude-generatedフラグ付きで実行
    let (stdout_without_generated, _) =
        run_codicat_with_args(&["--exclude-generated", "./"], Some(repo.path()))?;

    // 通常の出力には自動生成ファイルが含まれているか確認
    assert!(stdout_with_generated.contains("generated.rs"));
    assert!(stdout_with_generated.contains("This is generated code"));

    // --exclude-generatedを使った場合、自動生成ファイルが除外されているか確認
    assert!(!stdout_without_generated.contains("This is generated code"));

    // 一方で通常のファイルは両方の出力に含まれていることを確認
    assert!(stdout_with_generated.contains("a.txt"));
    assert!(stdout_without_generated.contains("a.txt"));
