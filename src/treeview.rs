...
fn build_tree_from_git<P: AsRef<Path>>(input_path: P) -> Result<TreeNode> {
    let abs_input = input_path.as_ref().canonicalize().context("Failed to resolve input path")?;
    
    let git_root = gitutil::get_git_root(&abs_input)?;
    
    let rel_input_path = abs_input.strip_prefix(&git_root)
        .unwrap_or_else(|_| Path::new("."))
        .to_path_buf();
    
    let git_files = gitutil::list_git_tracked_files(&git_root)?;
    
    let relevant_paths = git_files.iter()
        .filter_map(|file| {
...
fn insert_path(node: &mut TreeNode, path: &Path) {
    ...
    for (i, component) in components.iter().enumerate() {
        ...
        if !current.children.contains_key(component) {
            current.children.insert(component.clone(), TreeNode::new(component, is_file));
        }
...
/// ツリーの子ノードを再帰的に表示する
fn print_tree_children<W: Write>(
    node: &TreeNode,
    prefix: &str,
    writer: &mut W,
) -> Result<()> {
